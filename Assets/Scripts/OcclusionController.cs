...
public class OcclusionController : MonoBehaviour {
    ...
    private List<SceneryObject> _possibleOccludingObjects = new List<SceneryObject>();
    
    // for GUI
    private Dictionary<SceneryObject, Polygon> _distractorPolygons = new Dictionary<SceneryObject, Polygon>();
    ...
    // for raycast
    private HashSet<SceneryObject> _lastRayCastHits;
    private Dictionary<SceneryObject, HashSet<SceneryObject>> _lastRayCastDict;

    private void Start() {
        cam = Camera.main;
        _planes = GeometryUtility.CalculateFrustumPlanes(cam);
        _lastRayCastHits = new HashSet<SceneryObject>();
        _rayCastPositions = new List<Vector2>();
        _targetPolygons = new List<Polygon>();
        _lastRayCastDict = new Dictionary<SceneryObject, HashSet<SceneryObject>>();
    }
    ...
    private void OcclusionDetection(SceneryObject targetObject) {
    ...
    private void RayCaster(SceneryObject targetObject) {
        ...
        var newHits = new HashSet<SceneryObject>();
        var distanceDictionary = new Dictionary<SceneryObject, float>();
        ...
        for (var i = 0; i < screenPoints.Count; i++) {
            ...
            foreach (var hit in currentHits) {
                var so = hit.collider.GetComponent<SceneryObject>();
    ...
    private void PolygonDetection(SceneryObject targetObject) {
        ...
        var distractors = new List<SceneryObject>();
        var distances = new Dictionary<SceneryObject, float>();
    ...
    public void AddPossibleOccludingObject(SceneryObject so) {
        _possibleOccludingObjects.Add(so);
    }

    private bool IsObjectInViewFrustum(SceneryObject so) {
        return GeometryUtility.TestPlanesAABB(_planes, GetRendererBounds(so));
    }
    
    private static Bounds GetRendererBounds(SceneryObject so) {
    ...
    private void GetScreenAndScenePointsFromBoxCollider(SceneryObject so, ref List<Vector2> screenPositions,
    ...
    private void ExtentPoints(ref List<Vector2> screenPoints, ref Dictionary<Vector2, Vector3> scenePoints, SceneryObject targetObject) {
    ...
    private List<Vector2> GetScreenSpaceListFromRendererBounds(SceneryObject so) {
    ...
    private List<Vector2> GetScreenSpaceListFromBoxCollider(SceneryObject so) {
    ...
    private List<Vector2> GetScreenSpaceListFromMesh(SceneryObject so) {
        var returnList = new List<Vector2>();
